...
class MaCRO_DE(Optimizer):
    """
    MaCRO-DE:
    Mahalanobis-guided Adaptive Control in Differential Evolution
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.beta_min = self.validator.check_float(
            "beta_min", beta_min, (0.0, 2.0)
        )

        self.beta_max = self.validator.check_float(
            "beta_max", beta_max, (0.0, 2.0)
        )
    ...
    def evolve(self, epoch):
        ...
        div_norm_now = float(
            np.clip(
                div_awad / (self.div_max_seen + self.EPSILON),
                0.0,
                1.0
            )
        )
